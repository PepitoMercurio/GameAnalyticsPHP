...
$query = "SELECT id_user AS combined_id
FROM friend
WHERE id_friend = $id_user AND is_accept = TRUE
UNION
SELECT id_friend AS combined_id
FROM friend
WHERE id_user = $id_user AND is_accept = TRUE";
...
if ($result) {
    ...
    while ($row = pg_fetch_assoc($result)) {
        ...
        $userQuery = "SELECT gamertag, id FROM utilisateur WHERE id = $userId";
